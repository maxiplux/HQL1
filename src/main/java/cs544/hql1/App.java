...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Flight> flights = session.createQuery("select F from Flight as  F join F.origin  as destination join  F.airplane airplane   where destination.country = 'USA' "+

                " AND  airplane.capacity > 500 ").list();
        ...
        List<Airline> airlines = session.createQuery("select distinct A from Airline as A join A.flights as flights where flights.airplane.model = 'A380' ").list();
        ...
        flights = session.createQuery("select  F from Flight as F  join  F.airplane as airplane where  airplane.model = '747' and not F.airline.name = 'Star Alliance' ").list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
            System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
        ...
        Query query = session.createQuery("select F from Flight as F where F.departureDate =  '2009-08-07' and F.departureTime <  '12:00'");
        flights = query.list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
